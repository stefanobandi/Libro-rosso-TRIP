...
def genera_pdf_ufficiale(nome, qualifica, mese, anno, dati_mese, commenti_mese):
    ...
    for g in range(1, 32):
        ...
        comm = commenti_mese.get(g, "")[:18] # Tronca se troppo lungo
...
# (Uso del Form per la velocità come richiesto)
...
if selezione != "---":
    ...
    a_comp = st.sidebar.selectbox("Anno", [2025, 2026], index=0)

    with st.form("registro_form"):
        st.write(f"### Compilazione: {nome_corrente} ({m_comp} {a_comp})")
    ...
    if submit:
        # Qui inseriremo la logica di aggiornamento Session State già vista
        st.success("Dati pronti per l'esportazione!")

    # BOTTONE DOWNLOAD PDF
    st.divider()
    pdf_out = genera_pdf_ufficiale(nome_corrente, qual_corrente, m_comp, a_comp, temp_dati, temp_comm)
    st.download_button(
        label="📄 SCARICA REGISTRO PDF (X/R)",
        data=pdf_out,
        file_name=f"Registro_{nome_corrente}_{m_comp}.pdf",
